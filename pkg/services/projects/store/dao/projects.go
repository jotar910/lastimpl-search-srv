...
var (
	projectAllColumns            = []string{"id", "name", "description", "created_at", "updated_at"}
	projectColumnsWithoutDefault = []string{"name", "description", "created_at", "updated_at"}
...
func (q projectQuery) One(ctx context.Context, exec boil.ContextExecutor) (*Project, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "dao: failed to execute a one query for projects")
	}
...
func (q projectQuery) All(ctx context.Context, exec boil.ContextExecutor) (ProjectSlice, error) {
	...
	err := q.Bind(ctx, exec, &o)
	if err != nil {
		return nil, errors.Wrap(err, "dao: failed to assign all query results to Project slice")
	}
...
func (q projectQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "dao: failed to count projects rows")
	}
...
func (q projectQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "dao: failed to check if projects exists")
	}

	return count > 0, nil
}

// CodeFiles retrieves all the code_file's CodeFiles with an executor.
func (o *Project) CodeFiles(mods ...qm.QueryMod) codeFileQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"code_files\".\"project_id\"=?", o.ID),
	)

	query := CodeFiles(queryMods...)
	queries.SetFrom(query.Query, "\"code_files\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"code_files\".*"})
	}
...
func (o *Project) ProjectsTags(mods ...qm.QueryMod) projectsTagQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"projects_tags\".\"project_id\"=?", o.ID),
	)

	query := ProjectsTags(queryMods...)
	queries.SetFrom(query.Query, "\"projects_tags\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"projects_tags\".*"})
	}

	return query
}

// LoadCodeFiles allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (projectL) LoadCodeFiles(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProject interface{}, mods queries.Applicator) error {
	var slice []*Project
	var object *Project
	...
	query := NewQuery(
		qm.From(`code_files`),
		qm.WhereIn(`code_files.project_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load code_files")
	}

	var resultSlice []*CodeFile
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice code_files")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on code_files")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for code_files")
	}
	...
	if singular {
		object.R.CodeFiles = resultSlice
...
// LoadProjectsTags allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (projectL) LoadProjectsTags(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProject interface{}, mods queries.Applicator) error {
	var slice []*Project
	var object *Project
	...
	query := NewQuery(
		qm.From(`projects_tags`),
		qm.WhereIn(`projects_tags.project_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load projects_tags")
	}

	var resultSlice []*ProjectsTag
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice projects_tags")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on projects_tags")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for projects_tags")
	}
	...
	if singular {
		object.R.ProjectsTags = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &projectsTagR{}
			}
			foreign.R.Project = object
		}
		return nil
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.ProjectID {
				local.R.ProjectsTags = append(local.R.ProjectsTags, foreign)
				if foreign.R == nil {
					foreign.R = &projectsTagR{}
				}
				foreign.R.Project = local
...
// AddCodeFiles adds the given related objects to the existing relationships
// of the project, optionally inserting them as new records.
// Appends related to o.R.CodeFiles.
// Sets related.R.Project appropriately.
func (o *Project) AddCodeFiles(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*CodeFile) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.CodeFiles = append(o.R.CodeFiles, related...)
	}
...
// AddProjectsTags adds the given related objects to the existing relationships
// of the project, optionally inserting them as new records.
// Appends related to o.R.ProjectsTags.
// Sets related.R.Project appropriately.
func (o *Project) AddProjectsTags(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ProjectsTag) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.ProjectID = o.ID
	...
	if o.R == nil {
		o.R = &projectR{
			ProjectsTags: related,
		}
	} else {
		o.R.ProjectsTags = append(o.R.ProjectsTags, related...)
	}
...
func FindProject(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Project, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "dao: unable to select from projects")
	}
...
func (o *Project) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("dao: no projects provided for insertion")
	}
	...
	if err != nil {
		return errors.Wrap(err, "dao: unable to insert into projects")
	}
...
func (o *Project) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("dao: unable to update projects, could not build whitelist")
		}
	...
	if err != nil {
		return 0, errors.Wrap(err, "dao: unable to update projects row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dao: failed to get rows affected by update for projects")
	}
...
func (q projectQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "dao: unable to update all for projects")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dao: unable to retrieve rows affected for projects")
	}
...
func (o ProjectSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "dao: unable to update all in project slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dao: unable to retrieve rows affected all in update all project")
	}
	return rowsAff, nil
}
...
func (o *Project) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("dao: no projects provided for upsert")
	}
	if !boil.TimestampsAreSkipped(ctx) {
		currTime := time.Now().In(boil.GetLocation())
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("dao: unable to upsert projects, could not build update column list")
		}
	...
	if err != nil {
		return errors.Wrap(err, "dao: unable to upsert projects")
	}
...
func (o *Project) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if o == nil {
		return 0, errors.New("dao: no Project provided for delete")
	}
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "dao: unable to delete from projects")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dao: failed to get rows affected by delete for projects")
	}
...
func (q projectQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if q.Query == nil {
		return 0, errors.New("dao: no projectQuery provided for delete all")
	}
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "dao: unable to delete all from projects")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dao: failed to get rows affected by deleteall for projects")
	}
...
func (o ProjectSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "dao: unable to delete all from project slice")
	}
...
func (o *ProjectSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	if err != nil {
		return errors.Wrap(err, "dao: unable to reload all in ProjectSlice")
	}
...
func ProjectExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "dao: unable to check if projects exists")
	}
