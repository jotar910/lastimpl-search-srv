...
func testProjectToManyCodeFiles(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, codeFileDBTypes, false, codeFileColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, codeFileDBTypes, false, codeFileColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.CodeFiles().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadCodeFiles(ctx, tx, false, (*[]*Project)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.CodeFiles); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.CodeFiles = nil
	if err = a.L.LoadCodeFiles(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.CodeFiles); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testProjectToManyProjectsHistories(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, projectsHistoryDBTypes, false, projectsHistoryColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, projectsHistoryDBTypes, false, projectsHistoryColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.ProjectsHistories().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := ProjectSlice{&a}
	if err = a.L.LoadProjectsHistories(ctx, tx, false, (*[]*Project)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ProjectsHistories); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ProjectsHistories = nil
	if err = a.L.LoadProjectsHistories(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ProjectsHistories); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testProjectToManyProjectsTags(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, projectsTagDBTypes, false, projectsTagColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, projectsTagDBTypes, false, projectsTagColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.ProjectsTags().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := ProjectSlice{&a}
	if err = a.L.LoadProjectsTags(ctx, tx, false, (*[]*Project)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ProjectsTags); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ProjectsTags = nil
	if err = a.L.LoadProjectsTags(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ProjectsTags); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testProjectToManyAddOpCodeFiles(t *testing.T) {
	var err error
	...
	foreigners := []*CodeFile{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, codeFileDBTypes, false, strmangle.SetComplement(codeFilePrimaryKeyColumns, codeFileColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*CodeFile{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddCodeFiles(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.CodeFiles[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.CodeFiles[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.CodeFiles().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testProjectToManyAddOpProjectsHistories(t *testing.T) {
	var err error
	...
	var a Project
	var b, c, d, e ProjectsHistory

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, projectDBTypes, false, strmangle.SetComplement(projectPrimaryKeyColumns, projectColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*ProjectsHistory{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, projectsHistoryDBTypes, false, strmangle.SetComplement(projectsHistoryPrimaryKeyColumns, projectsHistoryColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*ProjectsHistory{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddProjectsHistories(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ProjectsHistories[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ProjectsHistories[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ProjectsHistories().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testProjectToManyAddOpProjectsTags(t *testing.T) {
	var err error
	...
	var a Project
	var b, c, d, e ProjectsTag

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, projectDBTypes, false, strmangle.SetComplement(projectPrimaryKeyColumns, projectColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*ProjectsTag{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, projectsTagDBTypes, false, strmangle.SetComplement(projectsTagPrimaryKeyColumns, projectsTagColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*ProjectsTag{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddProjectsTags(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ProjectsTags[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ProjectsTags[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ProjectsTags().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	projectDBTypes = map[string]string{`ID`: `integer`, `Name`: `character varying`, `Description`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)
